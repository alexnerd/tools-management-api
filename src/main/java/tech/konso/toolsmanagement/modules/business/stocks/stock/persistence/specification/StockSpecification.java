package tech.konso.toolsmanagement.modules.business.stocks.stock.persistence.specification;


import jakarta.persistence.criteria.Order;
import org.springframework.data.jpa.domain.Specification;
import tech.konso.toolsmanagement.modules.business.stocks.stock.persistence.dao.Stock;
import tech.konso.toolsmanagement.modules.business.stocks.stock.persistence.dao.Stock_;
...
/**
 * Class with specifications for {@link Stock}
 */
public final class StockSpecification {
     ...
     */
    public static Specification<Stock> likeSpec(String likeName) {
        ...
        return (root, criteriaQuery, criteriaBuilder) ->
                criteriaBuilder.like(criteriaBuilder.lower(root.get(Stock_.NAME)), likePattern);
    }
     ...
     * @return created specification
     */
    public static Specification<Stock> isArchivedSpec(boolean isArchived) {
        return (root, criteriaQuery, criteriaBuilder) ->
                criteriaBuilder.equal(root.get(Stock_.IS_ARCHIVED), isArchived);
    }

    /**
     * Specification for sorting stocks.
     ...
     * @return created specification
     */
    public static Specification<Stock> sortSpec(String sort) {
        return (root, criteriaQuery, criteriaBuilder) -> {
            Order descCreateDate = criteriaBuilder.desc(root.get(Stock_.CREATED_AT));
            ...
            switch (field.toLowerCase()) {
                case "name":
                    if (asc) criteriaQuery.orderBy(criteriaBuilder.asc(root.get(Stock_.NAME)));
                    else criteriaQuery.orderBy(criteriaBuilder.desc(root.get(Stock_.NAME)));
                    break;
                case "createdat":
                    if (asc) criteriaQuery.orderBy(criteriaBuilder.asc(root.get(Stock_.CREATED_AT)));
                    else criteriaQuery.orderBy(descCreateDate);
                    break;
                case "updatedat":
                    if (asc) criteriaQuery.orderBy(criteriaBuilder.asc(root.get(Stock_.UPDATED_AT)));
                    else criteriaQuery.orderBy(criteriaBuilder.desc(root.get(Stock_.UPDATED_AT)));
                    break;
