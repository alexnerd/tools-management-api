package tech.konso.toolsmanagement.modules.business.persons.role.persistence.specification;

import jakarta.persistence.criteria.Order;
import org.springframework.data.jpa.domain.Specification;
import tech.konso.toolsmanagement.modules.business.persons.role.persistence.dao.Role;
import tech.konso.toolsmanagement.modules.business.persons.role.persistence.dao.Role_;
...
/**
 * Class with specifications for {@link Role}
 */
public final class RoleSpecification {
     ...
     */
    public static Specification<Role> likeSpec(String likeName) {
        ...
        return (root, criteriaQuery, criteriaBuilder) ->
                criteriaBuilder.like(criteriaBuilder.lower(root.get(Role_.NAME)), likePattern);
    }
     ...
     * @return created specification
     */
    public static Specification<Role> isArchivedSpec(boolean isArchived) {
        return (root, criteriaQuery, criteriaBuilder) ->
                criteriaBuilder.equal(root.get(Role_.IS_ARCHIVED), isArchived);
    }

    /**
     * Specification for sorting roles.
     ...
     * @return created specification
     */
    public static Specification<Role> sortSpec(String sort) {
        return (root, criteriaQuery, criteriaBuilder) -> {
            Order descCreateDate = criteriaBuilder.desc(root.get(Role_.CREATED_AT));
            ...
            switch (field.toLowerCase()) {
                case "name":
                    if (asc) criteriaQuery.orderBy(criteriaBuilder.asc(root.get(Role_.NAME)));
                    else criteriaQuery.orderBy(criteriaBuilder.desc(root.get(Role_.NAME)));
                    break;

                case "createdat":
                    if (asc) criteriaQuery.orderBy(criteriaBuilder.asc(root.get(Role_.CREATED_AT)));
                    else criteriaQuery.orderBy(descCreateDate);
                    break;

                case "updatedat":
                    if (asc) criteriaQuery.orderBy(criteriaBuilder.asc(root.get(Role_.UPDATED_AT)));
                    else criteriaQuery.orderBy(criteriaBuilder.desc(root.get(Role_.UPDATED_AT)));
                    break;
