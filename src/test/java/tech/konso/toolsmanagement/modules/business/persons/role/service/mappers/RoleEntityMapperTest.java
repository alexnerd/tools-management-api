package tech.konso.toolsmanagement.modules.business.persons.role.service.mappers;


import org.junit.jupiter.api.BeforeAll;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.TestInstance;
import tech.konso.toolsmanagement.modules.business.persons.role.controller.dto.RoleRequest;
import tech.konso.toolsmanagement.modules.business.persons.role.persistence.dao.Role;

import static org.junit.jupiter.api.Assertions.assertEquals;

/**
 * Tests for RoleEntityMapper. Test for mapping fields and null values.
 * For every test, the given-when-then is used.
 * The given part sets the stage for the actual test and captures all prerequisites for executing the functionality
 * we want to test. The when part triggers the operation that we actually want to test. And in the then part, we
 * assert that the result that when the trigger produced is actually what we expect.
 * <p> Example:
 * <p> given - a calculator showing the number 2
 * <p> when - adding the number 3
 * <p> then - it should show the number 5
 */
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class RoleEntityMapperTest {

    private RoleEntityMapper mapper;

    @BeforeAll
    public void init() {
        mapper = new RoleEntityMapper();
    }

    /**
     * {@link RoleEntityMapper#toEntity(Role, RoleRequest)} should map {@link Role} name field.
     * Test creates object {@link RoleRequest} with non-null test field and try to map it to {@link Role} object.
     * Then checks by equality test field before mapping and after.
     */
    @Test
    public void map_to_role_should_map_name() {
        String roleName = "name";
        RoleRequest rq = RoleRequest.builder()
                .name(roleName)
                .build();

        Role role = mapper.toEntity(new Role(), rq);

        assertEquals(roleName, role.getName());
    }

    /**
     * {@link RoleEntityMapper#toEntity(Role, RoleRequest)} should map {@link Role} isArchived field.
     * Test creates object {@link RoleRequest} with non-null test field and try to map it to {@link Role} object.
     * Then checks by equality test field before mapping and after.
     */
    @Test
    public void map_to_role_should_map_is_archived() {
        boolean isArchived = true;
        RoleRequest rq = RoleRequest.builder()
                .isArchived(isArchived)
                .build();

        Role role = mapper.toEntity(new Role(), rq);

        assertEquals(isArchived, role.getIsArchived());
    }
}
